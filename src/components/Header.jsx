...
import DiariesDrawer from "./DiariesDrawer"; // რაკი ორივე components-შია, ერთი წერტილით შემოიტანე!

const Header = () => {
  const [isOpen, setIsOpen] = useState(false); // მობილურის მენიუსთვის
  const [isDiariesOpen, setIsDiariesOpen] = useState(false); // აი ეს სთეითი გინდა აუცილებლად!
    ...
    <>
      <header className="fixed top-0 left-0 w-full z-[100] bg-black/60 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center relative">
          ...
          {/* დესკტოპის ბათონი */}
          <button 
            onClick={() => setIsDiariesOpen(true)} 
            className="hidden md:block px-6 py-2 border border-red-700 text-red-700 text-[10px] uppercase tracking-widest hover:bg-red-700 hover:text-white transition-all cursor-pointer"
          >
            The Diaries
          </button>
          ...
          <div className={`fixed inset-0 h-screen w-full bg-black z-[100] transition-all duration-500 ease-in-out md:hidden flex flex-col items-center justify-center ${isOpen ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`}>
            ...
            <div className="mt-16">
              <button 
                onClick={() => {
                  setIsOpen(false);
                  setIsDiariesOpen(true);
                }}
                className="px-10 py-4 border border-red-700 text-red-700 text-xs uppercase tracking-[0.3em] hover:bg-red-700 hover:text-white transition-all font-bold cursor-pointer"
              >
                The Diaries
              </button>
            </div>
      ...
      {/* ჩავსვათ კომპონენტი სულ ბოლოში */}
      <DiariesDrawer 
        isOpen={isDiariesOpen} 
        onClose={() => setIsDiariesOpen(false)} 
      />
    </>
  );
};
